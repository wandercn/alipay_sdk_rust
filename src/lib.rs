...
mod tests {
    ...
    const PRIVATE_KEY_FILE: &str = "src/test/ffactory.org_私钥.txt"; // 沙箱环境自定义测试密钥
    const PUBILC_KEY_FILE: &str = "src/test/ffactory.org_公钥.txt"; // 沙箱环境自定义测试密钥
    ...
    fn test_sign() -> std::io::Result<()> {
        ...
        let public_key_str =
            std::fs::read_to_string(format!("{}/{}", project_dir, PUBILC_KEY_FILE))?;
