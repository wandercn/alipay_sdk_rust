...
mod tests {
    ...
    const PRIVATE_KEY_FILE:&str ="/Users/lsm/Documents/ffactory.org_私钥.txt";
    const PUBILC_KEY_FILE: &str ="/Users/lsm/Documents/ffactory.org_公钥.txt";
    ...
    fn test_sign() ->std::io::Result<()>{
        let private_key_str: String= std::fs::read_to_string(PRIVATE_KEY_FILE)?;
        let mut sign = builder().sign_type_rsa2().build();
        sign.set_private_key(&private_key_str)?;
        let public_key_str = std::fs::read_to_string(PUBILC_KEY_FILE)?;
